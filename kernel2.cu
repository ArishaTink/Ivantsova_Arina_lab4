...
int main()
{
    ...
    int channel_size = width * height * sizeof(unsigned char);
    unsigned char* h_channel_in = (unsigned char*)malloc(channel_size);
    unsigned char* h_channel_out = (unsigned char*)malloc(channel_size);
    unsigned char* result = (unsigned char*)malloc(width * height * channels);

    unsigned char* d_in, * d_out;
    cudaMalloc(&d_in, channel_size);
    cudaMalloc(&d_out, channel_size);
    ...
    for (int c = 0; c < channels; c++)
    {
        for (int i = 0; i < width * height; i++)
            h_channel_in[i] = img[i * channels + c];

        cudaMemcpy(d_in, h_channel_in, channel_size, cudaMemcpyHostToDevice);
        ...
        cudaMemcpy(h_channel_out, d_in, channel_size, cudaMemcpyDeviceToHost);

        for (int i = 0; i < width * height; i++)
            result[i * channels + c] = h_channel_out[i];
    }

    stbi_write_png("output.png", width, height, channels, result, width * channels);
    printf("let's gooo output.png (%dx%d)\n", width, height);
